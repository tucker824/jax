...
class LaxRandomTest(jtu.JaxTestCase):
  ...
  def testFoldIn(self):
    key = random.PRNGKey(0)
    keys = [random.fold_in(key, i).key for i in range(10)]
  ...
  def test_prng_seeds_and_keys(self, seed, type, jit, key):
    ...
    seed = type(seed)
    if jit:
      actual = api.jit(random.PRNGKey)(seed).key
    else:
      actual = random.PRNGKey(seed).key
  ...
  def test_prng_jit_invariance(self, seed, type):
    ...
    make_prng = lambda seed: random.PRNGKey(seed).key
  ...
  def test_random_split_doesnt_device_put_during_tracing(self):
    key = random.PRNGKey(1).key.block_until_ready()
